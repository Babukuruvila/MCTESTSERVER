...
async def next_page(bot, query):
    ...
    try:
        if settings['max_btn']:
            ...
            if n_offset == 0:
                ...
                btn.append([InlineKeyboardButton("▫️ Pages", callback_data="pages"), InlineKeyboardButton(f"{math.ceil(int(offset)/10)+1} / {math.ceil(total/10)}", callback_data="pages"), InlineKeyboardButton("Next ⏩", callback_data=f"next_{req}_{key}_{n_offset}")])
            else:
                btn.append(
                    [
            ...
            if n_offset == 0:
                ...
                btn.append([InlineKeyboardButton("▫️ Pages", callback_data="pages"), InlineKeyboardButton(f"{math.ceil(int(offset)/int(MAX_B_TN))+1} / {math.ceil(total/int(MAX_B_TN))}", callback_data="pages"), InlineKeyboardButton("Next ⏩", callback_data=f"next_{req}_{key}_{n_offset}")])
            else:
                btn.append(
                    [
                        InlineKeyboardButton("⏪ Back", callback_data=f"next_{req}_{key}_{off_set}"),
    ...
    btn.insert(0, [
        InlineKeyboardButton("🗣️ Lᴀɴɢᴜᴀɢᴇs 🗣️", callback_data=f"select_lang#{req}")
    ])
    try:
...
async def language_check(bot, query):
    ...
    if files:
        ...
        btn.insert(0, [
            InlineKeyboardButton("🗣️ Lᴀɴɢᴜᴀɢᴇs 🗣️", callback_data=f"select_lang#{userid}")
        ])
        ...
        if offset != "":
            ...
            try:
                if settings['max_btn']:
                    btn.append(
                        [InlineKeyboardButton("▫️ Pages", callback_data="pages"), InlineKeyboardButton(text=f"1/{math.ceil(int(total_results)/10)}",callback_data="pages"), InlineKeyboardButton(text="Next ⏩",callback_data=f"next_{req}_{key}_{offset}")]
                    )

                else:
                    btn.append(
                        [InlineKeyboardButton("▫️ Pages", callback_data="pages"), InlineKeyboardButton(text=f"1/{math.ceil(int(total_results)/int(MAX_B_TN))}",callback_data="pages"), InlineKeyboardButton(text="Next ⏩",callback_data=f"next_{req}_{key}_{offset}")]
                    )
            except KeyError:
                await save_group_settings(query.message.chat.id, 'max_btn', True)
                btn.append(
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        buttons = [[
                    InlineKeyboardButton('➕ Add Me To Your Groups ➕', url=f'http://t.me/{temp.U_NAME}?startgroup=join_otherbots&admin=post_messages+edit_messages+delete_messages+restrict_members+invite_users+pin_messages+promote_members+manage_video_chats+manage_chat')
                ],[
        ...
        buttons = [[
            InlineKeyboardButton('❔️Hᴏᴡ Tᴏ Usᴇ Mᴇ ❔️', callback_data='owner_info')
        ], [
        ...
        buttons = [[
            InlineKeyboardButton('Uᴘᴅᴀᴛᴇs Cʜᴀɴɴᴇʟ', url=CHNL_LNK),
            InlineKeyboardButton('Dɪsᴄʟᴀɪᴍᴇʀ', callback_data='source')
        ],[
            InlineKeyboardButton('Bᴀᴄᴋ', callback_data='start'),
...
async def auto_filter(client, msg, spoll=False):
    ...
    btn.insert(0, [
        InlineKeyboardButton("🗣️ Lᴀɴɢᴜᴀɢᴇs 🗣️", callback_data=f"select_lang#{message.from_user.id}")
    ])
